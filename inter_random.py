...
def inter_random_1k(G0, node_community_list, nswap=1, max_tries=100, connected=1):
    ...
    while swapcount < nswap:  # 有效交换次数小于规定交换次数
        ...
        if len(set([u, v, x, y])) == 4:  # 保证是四个独立节点
            if edge_in_community(node_community_list, (u, v)) == 0 and edge_in_community(node_community_list,
                                                                                         (x, y)) == 0:  # 保证所取的连边为社团内部连边
                if edge_in_community(node_community_list, (u, y)) == 0 and edge_in_community(node_community_list, (
                v, x)) == 0:  # 保证新生成的边还是社团内连边
...
G=inter_random_3k(G0,community_list_s,nswap=100, max_tries=20*M)
n1 = len(G.nodes())
m1 = len(G.edges())

print(n1)
print(m1)
nx.write_edgelist(G,'IEEE118_inter_3K.txt',data=False)
